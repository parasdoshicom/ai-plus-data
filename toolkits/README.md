# Toolkits Guide

Toolkits are practical templates, scorecards, and checklists for turning the playbooks into repeatable team behavior.

## Toolkits

- [`data-team-ai-rollout-checklist.md`](./data-team-ai-rollout-checklist.md): the safest first rollout checklist for a data team
- [`manager-ai-adoption-scorecard.md`](./manager-ai-adoption-scorecard.md): a manager-level scoring lens for progress, quality, and risk
- [`cdo-ai-operating-system-self-assessment.md`](./cdo-ai-operating-system-self-assessment.md): fast maturity read for senior leaders
- [`metric-skill-file-template.md`](./metric-skill-file-template.md): template for capturing recurring metric context and guardrails

## Suggested Order

1. Start with [`data-team-ai-rollout-checklist.md`](./data-team-ai-rollout-checklist.md) if you are rolling AI out.
2. Add [`manager-ai-adoption-scorecard.md`](./manager-ai-adoption-scorecard.md) if you need a recurring review cadence.
3. Use [`cdo-ai-operating-system-self-assessment.md`](./cdo-ai-operating-system-self-assessment.md) if you need the executive maturity lens.
