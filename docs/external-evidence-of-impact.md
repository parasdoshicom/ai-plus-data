# External Evidence Of Impact

This page collects **public, sourceable, externally visible signals** connected to the operating perspective behind this repo.

It is intentionally conservative.

The sources below connect the repo's themes to public evidence of operating scope, measurable outcomes, and external peer recognition.

## How To Read This Page

Not all public sources carry the same weight.

- official company or professional-body sources carry the most weight
- third-party case studies can provide strong evidence, especially when they include concrete metrics
- self-authored bios can be useful context, but they should not be treated as independent evidence

## Strongest External Signals

### 1. Third-party case study with measurable operating outcomes

Source:

- [Select Star: How Opendoor Leverages Amazon QuickSight and Select Star for Self-Service Analytics](https://www.selectstar.com/resources/amazon-quicksight-self-service-analytics)
- Repo read-through: [`case-studies/public-analytics-platform-transformation.md`](./case-studies/public-analytics-platform-transformation.md)

What it supports:

- stronger data trust and discoverability
- better analytics operating efficiency
- real platform and governance improvement, not just conceptual leadership

Publicly reported outcomes:

- time-to-triage reduced from 2 hours to 15 minutes
- internal NPS improved by 20%
- severe incidents dropped by 67%
- analytics costs were reduced by 80% year-over-year through the migration

Caveat:

This is a vendor-authored case study, so it should be read as a third-party evidence point, not an audited filing.

### 2. Official external professional recognition tied to senior operating scope

Source:

- [Institute of Analytics Fellow profile](https://ioaglobal.org/ioa-fellows/paras-doshi/)

What it supports:

- external recognition from a professional body
- seniority of role and scope
- credibility beyond self-published writing

What the profile states publicly:

- Paras is Head of Data at Opendoor
- the teams he leads power billions in real estate transactions
- his work spans marketing, pricing, and product analytics
- it has driven measurable growth and platform transformation

Caveat:

This is an official professional-body profile, which is strong for recognition and scope, but not the same as a detailed business case study.

### 3. Official company-authored technical scope

Source:

- [Opendoor: How Data Powers Opendoor’s AI Home Valuations](https://www.opendoor.com/articles/how-data-powers-opendoors-home-valuations)

What it supports:

- direct involvement in a real AI-native operating problem
- meaningful technical and business complexity
- public evidence that the perspective in this repo maps to a live production domain

What the article states publicly:

- Opendoor’s AI valuation models analyze 500+ property insights
- those models learn from 250,000+ real estate transactions
- they are powered by data from 20 external sources plus Opendoor’s proprietary transaction database

Caveat:

This is company-authored and Paras-authored, so it is best used as scope evidence rather than independent outcome evidence.

## Additional External Signals

### 4. Peer recognition in senior data leadership communities

Source:

- [Evanta San Francisco CDAO Community Governing Body](https://aaem-disp.evanta.com/cdao/san-francisco)

What it supports:

- recognition as a peer among senior data and analytics leaders
- relevance in CDO-level forums beyond personal publishing

Caveat:

This is a community leadership signal, not business outcome evidence.

### 5. Official awards and judging roles

Sources:

- [Globee Awards 2025 winners](https://globeeawards.com/2025-winners-artificial-intelligence-awards/)
- [Globee Awards 2025 judges](https://globeeawards.com/2025-judges-artificial-intelligence/)

What it supports:

- external recognition in AI and data leadership
- participation as both award recipient and evaluator

Caveat:

Awards help with external credibility, but they are weaker than operating results and should stay secondary to outcome evidence.

## Useful Context

These are helpful context sources, but they should not be treated as headline external evidence.

### 1. Self-authored public bio

Source:

- [Insight Extractor: About Paras Doshi](https://insightextractor.com/about-paras-doshi/)

Why it matters:

- it contains stronger detail on team size, impact, and leadership roles
- it helps explain the background behind the repo

Why it is not headline evidence:

- it is self-authored, so it should be treated as context unless corroborated elsewhere

## How These Sources Fit

The strongest public sources support three kinds of context:

- senior operating scope
- at least one third-party case study with concrete metrics
- external recognition beyond the repo itself

The sources do not replace interviews, references, or deeper diligence. They simply make the public record easier to inspect.

## Source Boundary

These are public sources, not confidential internal records or audited business filings. Vendor-authored and self-authored sources are labeled with caveats so readers can weight them appropriately.
